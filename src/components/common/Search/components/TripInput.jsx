    ...
    <div>
      ...
      {isActive && inputValue && (
        <ul className="absolute bg-slate-100 py-2 w-64 border-b border-gray cursor-pointer">
          {apiResults.length > 0 && apiResults.some((result) => result.nombres.toLowerCase().includes(inputValue.toLowerCase())) ? (
            apiResults.map((item) => (
              <li
          ...
          ) : inputValue && !apiResults.some((result) => result.nombres.toLowerCase().includes(inputValue.toLowerCase())) ? (
